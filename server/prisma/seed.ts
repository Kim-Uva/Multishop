  ...
  try {
    ...
    await prisma.producto.create({
      data: {
        nombre: "Xiaomi Mi 11",
        descripcion:
          "Teléfono con potente procesador Snapdragon y pantalla AMOLED.",
        precio: 120000,
        stock: 6,
        estadoProducto: false,
        idCategoria: 2,
    ...
    await prisma.producto.create({
      data: {
        nombre: "Razer Opus",
        descripcion:
          "Audífonos con sonido THX Certified y cancelación activa de ruido.",
        precio: 100000,
        stock: 10,
        estadoProducto: true,
        idCategoria: 5,
    ...
    for (const encabezadoCompraData of encabezadosCompraData) {
      ...
      const comprasDetalle = await Promise.all(
        encabezadoCompraData.detalleCompra.map(async (detalle) => {
